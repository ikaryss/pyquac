...
def test_iter_setup(x_key, y_key, x_min, x_max, y_min, y_max):
    ...
    if (x_mask is not None) and (y_mask is not None):
        ...
        print('LOAD\n', data.load)
        print('XMSK\n', x_mask)
        assert all(data.frequency == y_mask) and all(data.load == x_mask)

    elif (x_mask is not None) and (y_mask is None):
        print('LOAD\n', data.load)
        print('XMSK\n', x_mask)
        assert all(data.load == x_mask)

    if (x_mask is None) and (y_mask is not None):
        print('FREQ\n', data.frequency)
        print('YMSK\n', y_mask)
        assert all(data.frequency == y_mask)
